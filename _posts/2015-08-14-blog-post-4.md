...
It's easy to prove the equivalence $$H^h(X)=\text{Fil}^p H^h(X) \oplus \overline{	ext{Fil}^q H^h(X)} \quad \text { for every }(p, q) \text { with } p+q=h+1 .$$iff $$H^h(X, \mathbb{C})=\bigoplus_{p+q=k}  H^{p, q}, \text { with } \quad H^{p, q}=F^p\left(H^k\right) \cap \overline{F^q\left(H^k\right)}$$
